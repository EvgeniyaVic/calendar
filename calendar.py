...
#отрисовываем графический интерфейс
root = Tk()
root.title('Календарь') #заголовок окна
days = []              #для хранения полей таблицы
...
def fill():
    ...
    if month == 1:
        prew_month_day = calendar.monthrange(year - 1, 12)[1]
    else:
        prew_month_day = calendar.monthrange(year, month - 1)[1]
    ...
    for n in range(week_day):
        days[week_day - n - 1]['text'] = prew_month_day - n
